...
def wrapper_function_auditel(dataset: list[list[TemporalEvent]], apriori_function: FunctionType = call_classic_apriori, temporal_window: int = 0, 
    min_support: float = 0.2, min_confidence: float = 0.5, exponential_decay: bool = False, decay_rate=0.4, context_level: int = 0) -> list[Rule]:
    ...
    if context_level > 0:
        augmented_dataset = clean_context(augmented_dataset)
    ...
    auditel_frequent_patterns, frequent_itemsets_dict = find_auditel_patterns(frequent_itemsets, min_support=min_support)
    rules = find_auditel_rules(auditel_frequent_patterns, frequent_itemsets_dict, min_confidence=min_confidence)

    return rules

def wrapper_function_shop(dataset: list[list[TemporalEvent]], apriori_function: FunctionType = call_classic_apriori, temporal_window: int = 0, 
    min_support: float = 0.2, min_confidence: float = 0.5, exponential_decay: bool = False, decay_rate=0.4, ) -> list[Rule]:
    ...
    frequent_patterns, frequent_itemsets_dict = find_shop_patterns(frequent_itemsets, min_support=min_support)
    #for frequent_pattern in frequent_patterns:
    #    print(frequent_pattern)

    rules = find_shop_rules(frequent_patterns, frequent_itemsets_dict, min_confidence=min_confidence)

    return rules

def filter_diabetes_event_itemsets(frequent_itemsets: pd.DataFrame) -> pd.DataFrame:
    filtered_itemsets = pd.DataFrame(columns=['support', 'itemsets'])
    
    for row in frequent_itemsets.itertuples():
        itemset = row.itemsets
        flag = False
        for item in itemset:
            if not str(item).startswith('G'):
                flag = True
                break
        
        if flag:
            row = pd.DataFrame([[row.support, row.itemsets]], columns=['support', 'itemsets'])
            filtered_itemsets =pd.concat([filtered_itemsets, row])
            #filtered_itemsets = filtered_itemsets.append(frequent_itemsets.iloc[i])
    return filtered_itemsets

def wrapper_function_diabets(dataset: list[list[TemporalEvent]], apriori_function: FunctionType = call_classic_apriori, temporal_window: int = 0, 
    min_support: float = 0.2, min_confidence: float = 0.5, exponential_decay: bool = False, decay_rate=0.4, context_level: int = 0) -> list[Rule]:
    ...
    frequent_itemsets = filter_diabetes_event_itemsets(frequent_itemsets) #only itemsets with at least one diabetes event
    auditel_frequent_patterns, frequent_itemsets_dict = find_diabetes_patterns(frequent_itemsets, min_support=min_support)
    rules = find_diabetes_rules(auditel_frequent_patterns, frequent_itemsets_dict, min_confidence=min_confidence)
